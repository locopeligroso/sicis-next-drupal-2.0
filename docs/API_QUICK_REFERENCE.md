...
|------|----------|----------|-----|
| **C1** | `GET /{locale}/api/v1/entity?path={path}` | `fetchEntity(path, locale)` | 60s |
| **C2** | `GET /{locale}/api/v1/translate-path?path=&from=&to=` | `getTranslatedPath(path, from, to)` | 3600s |

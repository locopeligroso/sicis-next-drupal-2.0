...
| `showroom`     | `Showroom.tsx`    | Legacy      | No                     | **No `field_blocchi`** — Drupal returns 400 if included. Key fields: `field_indirizzo`, `field_citta`, `field_area`, `field_telefono`, `field_indirizzo_email`, `field_collegamento_gmaps`, `field_collegamento_esterno`, `field_gallery`. showrooms endpoint normalizes `gmapsUrl` → `mapsUrl`. |
...
| ---------------- | ------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `categoria`      | `Categoria.tsx`     | Legacy      | 3-branch dispatch based on `getCategoriaProductType()`: (1) product listing via `ProductListingTemplate` when a `filterField` is configured, (2) subcategory grid via subcategories when no filter but has NID children, (3) page grid via pages-by-category. Secondary fetches: subcategories (`fetchSubcategories`), products ⚠️ LEGACY (`fetchProducts`), pages-by-category (`fetchPagesByCategory`). All use `node._nid` (integer NID) as the lookup key. |
| `categoria_blog` | `CategoriaBlog.tsx` | Legacy+Para | `title` + `field_immagine` + `body` + `field_blocchi`.                                                                                                                                                                                                                                                                                                                                                                                                        |
| `documento`      | `Documento.tsx`     | Legacy      | **No `field_blocchi`** — Drupal returns 400 if included. Key field: `field_allegato` (file download, shape: `{ entity: { uri: { value: string } } }`). Currently 0 items published on staging (documents endpoint returns empty).                                                                                                                                                                                                                             |
...
All 11 vocabularies registered in `TaxonomyTypeName` and consumed via the taxonomy endpoint.

| Vocabulary             | Product Type | API Endpoint | Has Images     | Filter Priority | Notes                                                                                                                                                                               |
| ---------------------- | ------------ | ------------ | -------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mosaico_collezioni`   | Mosaico      | taxonomy     | Yes            | P0 (path-based) | Terms have `field_immagine`, full collection specs (dimensions, thickness, resistance fields). Used as `field_collezione` on `prodotto_mosaico`. Template: `MosaicoCollezione.tsx`. |
| `mosaico_colori`       | Mosaico      | taxonomy     | Yes (swatches) | P0 (path-based) | Swatch images. Template: `MosaicoColore.tsx`.                                                                                                                                       |
| `vetrite_collezioni`   | Vetrite      | taxonomy     | Yes            | P0 (path-based) | Full collection specs: dimensions, thicknesses, special glass fields, documents array. Template: `VetriteCollezione.tsx`.                                                           |
| `vetrite_colori`       | Vetrite      | taxonomy     | Yes            | P0 (path-based) | Template: `VetriteColore.tsx`.                                                                                                                                                      |
| `vetrite_finiture`     | Vetrite      | taxonomy     | No             | P1 (sidebar)    | No images. Fallback template: `TaxonomyTerm.tsx` (wireframe).                                                                                                                       |
| `vetrite_textures`     | Vetrite      | taxonomy     | No             | P1 (sidebar)    | No images. Fallback template: `TaxonomyTerm.tsx`.                                                                                                                                   |
| `arredo_finiture`      | Arredo       | taxonomy     | —              | P1 (sidebar)    | **Empty vocabulary — 0 terms in Drupal.** taxonomy endpoint returns empty array.                                                                                                    |
| `tessuto_colori`       | Tessuto      | taxonomy     | No             | P1 (sidebar)    | Fallback template: `TaxonomyTerm.tsx`.                                                                                                                                              |
| `tessuto_finiture`     | Tessuto      | taxonomy     | —              | P1 (sidebar)    | **Empty vocabulary — 0 terms in Drupal.** taxonomy endpoint returns empty array.                                                                                                    |
| `tessuto_tipologie`    | Tessuto      | taxonomy     | No             | P1 (sidebar)    | Fallback template: `TaxonomyTerm.tsx`.                                                                                                                                              |
| `tessuto_manutenzione` | Tessuto      | taxonomy     | No             | Display only    | Not used as a filter. Displayed on product detail pages. Fallback template: `TaxonomyTerm.tsx`.                                                                                     |

**taxonomy response item shape for all vocabularies:**
...
Note: The taxonomy response has **no `path` field**. Slugs are derived client-side via `deriveSlug(name)` (slugify with NFC normalization). Explicit overrides are in `SLUG_OVERRIDES` within `src/domain/filters/registry.ts`.
...
| ------------------------------------- | ----------------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `taxonomy_term--mosaico_collezioni`   | `MosaicoCollezione.tsx` | Legacy listing | taxonomy (all filter options for sidebar) + products ⚠️ LEGACY (products filtered by this collection)                                   |
| `taxonomy_term--mosaico_colori`       | `MosaicoColore.tsx`     | Legacy listing | taxonomy + products ⚠️ LEGACY                                                                                                           |
| `taxonomy_term--vetrite_collezioni`   | `VetriteCollezione.tsx` | Hybrid         | taxonomy + products ⚠️ LEGACY + documents extracted from the term's own `field_documenti` array (resolved inline from entity ⚠️ LEGACY) |
| `taxonomy_term--vetrite_colori`       | `VetriteColore.tsx`     | Legacy listing | taxonomy + products ⚠️ LEGACY                                                                                                           |
...
These shapes recur across multiple content types. All come from the entity endpoint ⚠️ LEGACY (`fetchEntity`), which returns fully pre-resolved entities with all relationships inline.
...
### Image field (entity ⚠️ LEGACY endpoint response)

```typescript
// Shape inside entity ⚠️ LEGACY data:
{
  type: "file--file",
...
// In adapters:
const href =
  getTextValue(p.field_collegamento_esterno) ?? // handles string case
...
### Entity reference (resolved inline in entity ⚠️ LEGACY)

```typescript
// entity ⚠️ LEGACY returns fully resolved relationships — no secondary fetch needed.
...
### Date field (blog endpoint)

```typescript
// blog endpoint returns Unix timestamp as string:
created: '1772451555';
...
// Exception — products ⚠️ LEGACY endpoint returns priceOnDemand as string:
...
| ------------------------------------------------ | ------------------------------------------------ | --------------------------------- | -------------------------------------------------------------------- |
| `"https://www.sicis-stage.com/it/mosaico/pluma"` | `"/it/mosaico/pluma"`                            | `stripDomain()`                   | All Views responses (products–pages-by-category) `path` fields       |
...
| `"1772451555"` (Unix timestamp string)           | `"2026-03-25T..."` (ISO 8601)                    | `unixToIso()`                     | blog endpoint `created` field                                        |
| `"0"` / `"1"` / `null` (string)                  | Compared as string or cast inline                | inline cast                       | products ⚠️ LEGACY `priceOnDemand` field                             |
| `{ uri: { url: "https://..." } }` (entity image) | `"https://..."` (absolute URL string)            | `getDrupalImageUrl()`             | All entity ⚠️ LEGACY image fields in templates and adapters          |
| `field_collegamento_gmaps`                       | `mapsUrl`                                        | field rename in showrooms fetcher | Showroom cards                                                       |
| `weight: "3"` (string)                           | Not cast — kept as string                        | none                              | taxonomy term weight                                                 |
...
Exception: The menu endpoint uses a different URL pattern (`/api/menu/` without `v1`) and calls `fetch()` directly — it does not go through `apiGet()`.

### Slug derivation for taxonomy terms

taxonomy endpoint terms have no `path` or `slug` field. Slug is derived client-side:
...
| Endpoint           | Path Pattern                                                                  | Function                 | Response Type                                     | Revalidate |
| ------------------ | ----------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------- | ---------- |
| entity ⚠️ LEGACY   | `/{locale}/api/v1/entity?path={path}`                                         | `fetchEntity()`          | `EntityResponse`                                  | 60 s       |
| translate-path     | `/{locale}/api/v1/translate-path?path={path}&from={locale}&to={targetLocale}` | `getTranslatedPath()`    | `TranslatePathResponse`                           | 3600 s     |
| products ⚠️ LEGACY | `/{locale}/api/v1/products/{productType}`                                     | `fetchProducts()`        | `PaginatedResponse<ProductCard>`                  | 60 s       |
| product-counts     | `/{locale}/api/v1/products/{productType}/counts/{filterKey}`                  | `fetchFilterCounts()`    | `CountsResponse`                                  | 60 s       |
| taxonomy           | `/{locale}/api/v1/taxonomy/{vocabulary}`                                      | `fetchFilterOptions()`   | `TaxonomyTermItem[]`                              | 3600 s     |
| category-options   | `/{locale}/api/v1/category-options/{productType}`                             | `fetchCategoryOptions()` | `{ items: CategoryOptionItem[] }` (no pagination) | 3600 s     |
| blog               | `/{locale}/api/v1/blog`                                                       | `fetchBlogPosts()`       | `PaginatedResponse<BlogCard>`                     | 300 s      |
| projects           | `/{locale}/api/v1/projects`                                                   | `fetchProjects()`        | `PaginatedResponse<ProjectCard>`                  | 300 s      |
| environments       | `/{locale}/api/v1/environments`                                               | `fetchEnvironments()`    | `PaginatedResponse<EnvironmentCard>`              | 300 s      |
| showrooms          | `/{locale}/api/v1/showrooms`                                                  | `fetchShowrooms()`       | `ShowroomCard[]` (no pagination)                  | 300 s      |
| documents          | `/{locale}/api/v1/documents`                                                  | `fetchDocuments()`       | `PaginatedResponse<DocumentCard>`                 | 300 s      |
| subcategories      | `/{locale}/api/v1/subcategories/{parentNid}`                                  | `fetchSubcategories()`   | `CategoryCard[]`                                  | 300 s      |
| pages-by-category  | `/{locale}/api/v1/pages-by-category/{parentNid}`                              | `fetchPagesByCategory()` | `PaginatedResponse<PageCard>`                     | 300 s      |
| menu               | `/{locale}/api/menu/{menuName}`                                               | `fetchMenu()`            | Menu tree                                         | 600 s      |

**subcategories and pages-by-category require integer NID** (`node._nid` from entity response), not UUID.

**products (legacy) filter limitations:**

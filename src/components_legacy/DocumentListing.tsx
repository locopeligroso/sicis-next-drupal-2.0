...
function DocumentCardItem({
  ...
  const internalHref = document.path ? `/${locale}${document.path}` : null;
  const href = document.externalUrl || internalHref;
  const isExternal = !!document.externalUrl;

  const typeLabel = document.documentType ?? null;
  const linkLabel = isExternal ? 'Scarica' : 'Apri';
  ...
  return (
    <div style={{ maxWidth: '80rem', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      {documents.length > 0 ? (
        ...
        >
          {documents.map((doc) => (
            <DocumentCardItem key={doc.id} document={doc} locale={locale} />
          ))}
        </div>
      ) : (
        <div

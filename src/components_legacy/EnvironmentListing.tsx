...
function EnvironmentCardItem({
  environment,
  locale,
}: {
  environment: AmbienteCard;
  locale: string;
}) {
  const href = environment.path ? `/${locale}${environment.path}` : null;
...
export default function EnvironmentListing({
  title,
  environments,
  total,
  locale,
      ...
      >
        ...
        {total > 0 && (
          <p style={{ fontSize: '0.875rem', color: '#6b7280' }}>
            {total} {total === 1 ? 'ambiente' : 'ambienti'}
      ...
      {environments.length > 0 ? (
        ...
        >
          {environments.map((environment) => (
            <EnvironmentCardItem key={environment.id} environment={environment} locale={locale} />
          ))}
        </div>
      ) : (
        <div

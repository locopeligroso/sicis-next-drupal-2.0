...
const TYPE_LABELS: Record<BlogCard['type'], string> = {
  articolo: 'Articolo',
  news: 'News',
  tutorial: 'Tutorial',
};
...
function BlogCardItem({
  ...
  const href = post.path ? `/${locale}${post.path}` : null;
  ...
  const card = (
    ...
    >
      ...
      <div style={{ padding: '0.75rem' }}>
        ...
        >
          {formatDate(post.created, locale)}
        </p>
        <p
          style={{
            fontSize: '0.875rem',
  ...
  return (
    <div style={{ maxWidth: '80rem', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      {posts.length > 0 ? (
        ...
        >
          {posts.map((post) => (
            <BlogCardItem key={post.id} post={post} locale={locale} />
          ))}
        </div>
      ) : (
        <div

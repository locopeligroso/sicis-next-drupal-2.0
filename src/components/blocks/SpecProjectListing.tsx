...
export interface SpecProjectListingProps {
  title: string;
...
function CategoryPills({
  ...
  return (
    ...
    >
      ...
      {categories.map((cat) => {
        const isActive = activeCategory === cat.tid;
        return (
          <a
            key={cat.tid}
...
function ProjectCard({
  ...
  const isExternal = project.path?.startsWith('http') ?? false;
  const href = project.path
    ? isExternal
      ? project.path
      : `/${locale}${project.path}`
    : null;
...
export function SpecProjectListing({
  title,
  projects,
  categories,
  activeCategory,

/**
 * Server-side product fetcher for categoria listing pages.
 * Fetches products from Drupal JSON:API by product type.
 */
...
// Use only the origin (protocol + host) to avoid double-path when prepending
// Drupal's uri.url which is already root-relative (e.g. /www.sicis.../sites/...)
const PUBLIC_DRUPAL_URL = (() => {
  const base = process.env.NEXT_PUBLIC_DRUPAL_BASE_URL || 'http://localhost';
 ...
 */
function slugToTermName(slug: string): string {
  ...
  try {
    ...
    for (const item of json.included ?? []) {
      if (item.type === 'file--file') {
        ...
        if (uriUrl) {
          fileMap.set(item.id, `${PUBLIC_DRUPAL_URL}${uriUrl}`);
        }
      }
    }

/**
 * Fetches taxonomy term options from Drupal JSON:API for filter UI population.
 * Cached aggressively (revalidate: 3600) — taxonomy changes rarely.
 */
 ...
 * in fetch-products.ts reconstructs the term name from a URL slug.
 *
  ...
  try {
    ...
    return (json.data ?? []).map((item: Record<string, unknown>) => {
      ...
      return {
        ...
        // in fetch-products.ts: slug → title capitalised matches field_categoria.title
 ...
 * Mirrors the inverse of `slugToTermName` in fetch-products.ts so that

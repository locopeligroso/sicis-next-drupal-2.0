...
export default async function MosaicoCollezione({
  node,
  currentPage = 1,
  pageSize = 48,
  basePath = '',
  ...
  const [filterOptions, { products, total }] = await Promise.all([
    ...
    fetchProducts({
      ...
      // Collections with subcollections (e.g. NeoColibrì → Barrels/Cubes/Domes)
      // use STARTS_WITH so all child products are included.
      // Plus any P1/P2 query filters parsed from URL.
      filters: [
        {
          field: 'field_collezione.name',
          value: name,
          operator: name.startsWith('NeoColibrì') || name.startsWith('Neocolibrì')
            ? 'STARTS_WITH'
            : '=',
        },
        ...parsedFilters.filterDefinitions.filter(d => d.field !== 'field_collezione.name'),
      ],
    }),
  ]);

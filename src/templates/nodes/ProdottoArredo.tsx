...
import type { ProdottoArredo as ProdottoArredoType } from '@/types/drupal/products/arredo';
...
// ── Component ─────────────────────────────────────────────────────────────────
export default async function ProdottoArredo({ node }: { node: Record<string, unknown> }) {
  ...
  const ftpFiles = (typedNode as Record<string, unknown>).field_path_file_ftp as string[] | string | undefined;
  ...
  return (
    <article style={{ maxWidth: '60rem', margin: '0 auto', padding: '2rem' }}>
      ...
      <nav aria-label="Breadcrumb" style={{ marginBottom: '1.5rem' }}>
        <ol style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexWrap: 'wrap', gap: '0.25rem', fontSize: '0.8125rem', color: '#888' }}>
          <li>
            <Link href={`/${locale}/arredo`} style={{ color: '#888', textDecoration: 'none' }}>{t('furniture')}</Link>
          </li>
          {categoriaName && (
            <>
              ...
              <li>
                {categoriaPath ? (
                  <Link href={`/${locale}${categoriaPath}`} style={{ color: '#888', textDecoration: 'none' }}>{categoriaName}</Link>
                ) : (
          ...
          {title && (
            <>
              <li aria-hidden="true">/</li>
              <li aria-current="page" style={{ color: '#333' }}>{title}</li>
            </>
          )}
        </ol>
      </nav>
      ...
      {finiture.length > 0 && (
        <section className={styles.section} aria-labelledby="finiture-heading">
          <h2 id="finiture-heading" className={styles.sectionHeading}>{t('colorsAndFinishes')}</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(12rem, 1fr))', gap: '1rem' }}>
            {finiture.map((f, i) => {
              ...
              return (
                <div key={f.id ?? i} style={{ padding: '0.75rem', border: '0.0625rem solid #e0e0e0' }}>
                  {!!f.field_immagine && (
                    <DrupalImage field={f.field_immagine} alt={etichetta || f.name || ''} aspectRatio="1" style={{ marginBottom: '0.5rem' }} />
                  )}
                  <p style={{ margin: '0 0 0.125rem', fontWeight: 600, fontSize: '0.9375rem' }}>{etichetta || f.name}</p>
                  {testo && <p style={{ margin: 0, fontSize: '0.8125rem', color: '#666' }}>{testo}</p>}
                </div>
              );
            })}
          </div>
        ...
        <section className={styles.section} aria-labelledby="prezzo-heading">
          ...
          {prezzoOnDemand ? (
            ...
            <div style={{ display: 'flex', gap: '2rem', alignItems: 'baseline' }}>
              {prezzoEu && <p style={{ margin: 0, fontSize: '1.5rem', fontWeight: 700 }}>{Number(prezzoEu).toLocaleString('it-IT')}€</p>}
              {prezzoUsa && <p style={{ margin: 0, fontSize: '1.5rem', fontWeight: 700 }}>{Number(prezzoUsa).toLocaleString('en-US')}$</p>}
            </div>
          )}
        </section>
      )}
      ...
      {categoriaData && (
        <section className={styles.section} aria-labelledby="categoria-heading">
          <h2 id="categoria-heading" className={styles.sectionHeading}>{t('category')}</h2>
          {categoriaName && (
            categoriaPath ? (
              <Link href={`/${locale}${categoriaPath}`} style={{ fontWeight: 600, fontSize: '1.125rem', color: '#111', textDecoration: 'none', display: 'block', marginBottom: '0.5rem' }}>
                {categoriaName}
              </Link>
            ) : (
              <p style={{ fontWeight: 600, fontSize: '1.125rem', margin: '0 0 0.5rem' }}>{categoriaName}</p>
            )
          )}
          <DrupalImage
            ...
            {documenti.map((doc, i) => {
              ...
              return (
                <li key={doc.id ?? i} className={styles.docItem}>
                  ...
                  {href && (
                    <a href={href} target="_blank" rel="noopener noreferrer" aria-label={`${tCommon('download')} ${docTitolo ?? 'documento'}`}
      ...
      {(file3d || extLink) && (
        <section className={styles.section} style={{ marginBottom: 0 }} aria-labelledby="extra-heading">
          <h2 id="extra-heading" className={styles.sectionHeading}>{t('linksAndResources')}</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {file3d && (
              <a href={`/sites/default/files/${file3d}`} target="_blank" rel="noopener noreferrer"
                style={{ fontSize: '0.875rem', color: '#333', textDecoration: 'underline' }}>
                {t('download3dFile')}
              </a>
            )}
            {extLink && (

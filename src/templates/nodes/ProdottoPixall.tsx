...
// ── Component ─────────────────────────────────────────────────────────────────
export default async function ProdottoPixall({
  node,
}: {
  ...
  // ── Numeric fields ───────────────────────────────────────────────────────────
  const consumoM2 =
    typedNode.field_consumo_stucco_m2 != null
      ? String(typedNode.field_consumo_stucco_m2)
      : null;
  const consumoSqft =
    typedNode.field_consumo_stucco_sqft != null
      ? String(typedNode.field_consumo_stucco_sqft)
      : null;

  // ── New attribute fields ─────────────────────────────────────────────────────
  const retinatura = (typedNode.field_retinatura ?? undefined) as
    | string
    | undefined;
  const utilizzi = getProcessedText(typedNode.field_utilizzi);
  const numeroModuli = (typedNode.field_numero_moduli ?? undefined) as
    | string
    | number
    | undefined;
  ...
  const gallery = [...galleryIntro, ...(typedNode.field_gallery ?? [])];
  ...
  // ── Derived flags ────────────────────────────────────────────────────────────
  const hasDimensions =
    dimTesseraMm || dimTesseraInch || dimFoglioMm || dimFoglioInch || dimModuli;
  ...
  return (
    ...
    <article>
          ...
          {hasDimensions && (
            <div
              style={{
                ...
                gap: '1.25rem',
                marginBottom:
                  hasConsumption || retinatura || numeroModuli ? '1.5rem' : 0,
              }}
            >
              {dimTesseraMm && (
                <div>
      ...
      {immagineModuli ? (
        <section
          className={styles.section}
          aria-labelledby="moduli-img-heading"
        >

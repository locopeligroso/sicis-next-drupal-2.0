...
export default async function ProdottoMosaico({ node }: { node: Record<string, unknown> }) {
  ...
  heroSlides.push({ type: 'static', src: '/images/usa-mosaic-quality.jpg', alt: 'Quality certification' });
  ...
  // ── Price ──
  const heroPrice = prezzoUsa
    ? `$${prezzoUsa}`
    : prezzoEu
      ? `€${prezzoEu}`
      : null;
  const heroPriceUnit = prezzoUsa ? '/sqft' : prezzoEu ? '/m²' : undefined;

  // ── Details Block data ──
  // Dimensions + physical properties go in the attributes row
  const detailAttributes: AttributeItem[] = [
    ...(collezioneData?.field_dimensione_foglio_inch ? [{ label: 'Sheet size', value: collezioneData.field_dimensione_foglio_inch }] : []),
    ...(collezioneData?.field_dimensione_tessera_inch ? [{ label: 'Chip size', value: collezioneData.field_dimensione_tessera_inch }] : []),
    ...(collezioneData?.field_spessore_inch ? [{ label: 'Thickness', value: collezioneData.field_spessore_inch }] : []),
  ...
  const boolLabel = (v: unknown) => v === true ? t('resistant') : v === false ? t('absent') : null;
  ...
  const rawDocs = (collezioneData?.field_documenti as Array<{ id?: string; title?: unknown; field_titolo_main?: unknown; field_tipologia_documento?: unknown; field_collegamento_esterno?: unknown; field_immagine?: unknown; field_allegato?: { entity?: { uri?: { value?: string } } } }> | undefined) ?? [];
  ...
  for (const doc of rawDocs) {
    const docTitle = getTextValue(doc.field_titolo_main) || getTextValue(doc.title) || '';
    ...
    const isGuide = docTitle.toLowerCase().includes('install') || docTitle.toLowerCase().includes('manual') || docType?.toLowerCase().includes('guide');
    // "Cosa rende unici" / "what makes unique" goes to hero discover link
    const isDiscover = docTitle.toLowerCase().includes('rende unic') || docTitle.toLowerCase().includes('makes unique');
  ...
  return (
    <article className="flex flex-col gap-(--spacing-section) py-(--spacing-section)">
      ...
      <ProductSpecs
        ...
        groutingImageSrc={stucchi.length > 0 ? getDrupalImageUrl((stucchi[0] as Record<string, unknown>).field_immagine) : undefined}
        groutConsumption={collezioneData?.field_consumo_stucco_m2 != null ? `${collezioneData.field_consumo_stucco_m2} kg/m²` : undefined}

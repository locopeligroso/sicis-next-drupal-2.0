...
// ── Component ─────────────────────────────────────────────────────────────────
export default async function ProdottoTessuto({ node }: { node: Record<string, unknown> }) {
  ...
  const composizione = getProcessedText((typedNode as Record<string, unknown>).field_composizione);
  ...
  const finiture = (Array.isArray(typedNode.field_finiture_tessuto)
    ? typedNode.field_finiture_tessuto
    : typedNode.field_finiture_tessuto
      ? [typedNode.field_finiture_tessuto]
      : []) as FinituraItem[];
  const tipologie = (Array.isArray(typedNode.field_tipologia_tessuto)
  ...
  // ── Derived ───────────────────────────────────────────────────────────────
  const hasDimensions = altezzaCm || altezzaInch || peso;
  const tipologiaLabel = tipologie.map(t => t.name).filter(Boolean).join(' · ');
    ...
    <article style={{ maxWidth: '60rem', margin: '0 auto', padding: '2rem' }}>
      ...
      <nav aria-label="Breadcrumb" style={{ marginBottom: '1.5rem' }}>
        <ol style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexWrap: 'wrap', gap: '0.25rem', fontSize: '0.8125rem', color: '#888' }}>
          <li>
            <Link href={`/${locale}/tessile/tessuti`} style={{ color: '#888', textDecoration: 'none' }}>
              {t('fabrics')}
            </Link>
          </li>
          {categoriaName && (
            <>
              ...
              <li>
                {categoriaPath ? (
                  <Link href={`/${locale}${categoriaPath}`} style={{ color: '#888', textDecoration: 'none' }}>
                    {categoriaName}
                  </Link>
                ) : (
      ...
      {tipologiaLabel && (
        <p style={{ fontSize: '0.875rem', color: '#888', margin: '0 0 1.5rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          {tipologiaLabel}
        </p>
      )}
      ...
      {(hasDimensions || utilizzo) && (
        <section className={styles.section} aria-labelledby="specifiche-heading">
          <h2 id="specifiche-heading" className={styles.sectionHeading}>{t('technicalSpecs')}</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(10rem, 1fr))', gap: '1.25rem' }}>
            ...
            {utilizzo && (
              <div>
                ...
                <p style={{ margin: 0, fontWeight: 500, textTransform: 'capitalize' }}>{utilizzo}</p>
              </div>
            )}
          </div>
        </section>
          ...
          {categoriaName && (
            categoriaPath ? (
              <Link
                ...
                style={{ fontWeight: 600, fontSize: '1.125rem', color: '#111', textDecoration: 'none', display: 'block', marginBottom: '0.75rem' }}
              >
                {categoriaName}
              </Link>
            ) : (
              <p style={{ fontWeight: 600, fontSize: '1.125rem', margin: '0 0 0.75rem' }}>{categoriaName}</p>
            )
          )}
          <DrupalImage
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(12rem, 1fr))', gap: '1rem' }}>
            {finiture.map((f, i) => {
              ...
              return (
                <div
                  key={f.id ?? i}
                  style={{ padding: '0.75rem', border: '0.0625rem solid #e0e0e0' }}
                >
                  {codice && (
                    <span
                      aria-hidden="true"
                      style={{
                        ...
                        height: '2rem',
                        borderRadius: '50%',
                        background: codice.startsWith('#') ? codice : `#${codice}`,
                        border: '0.0625rem solid rgba(0,0,0,0.12)',
                        marginBottom: '0.5rem',
                      }}
                    />
                  )}
                  <p style={{ margin: '0 0 0.125rem', fontWeight: 600, fontSize: '0.9375rem' }}>
                    {etichetta || f.name}
                  </p>
                  {testo && (
                    <p style={{ margin: 0, fontSize: '0.8125rem', color: '#666' }}>{testo}</p>
                  )}
                </div>
              );
            })}
      ...
      {manutenzione.length > 0 && (
        <section className={styles.section} aria-labelledby="manutenzione-heading">
          <h2 id="manutenzione-heading" className={styles.sectionHeading}>{t('maintenanceInstructions')}</h2>
      ...
      {/* ── 10. Documenti ────────────────────────────────────────────────────── */}
      {documenti.length > 0 && (
        <section className={styles.section} style={{ marginBottom: 0 }} aria-labelledby="documenti-heading">
